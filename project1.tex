...
%However, these methods may be deemed unsatisfying as it is unclear whether these types of strategies are guaranteed to perform well for goals/rewards which are likely to be specified.
%Commonly-used examples, such as empowerment , take no heed of the process by which future goals/rewards will be constructed.
%This renders such methods unable to learn about this aspect of the environment and improve their preparation strategies in an episodic framework.
...
\subsection{Limitations of the empowerment heuristic}
...
[TODO: Express what \cite{singh2010intrinsically} suggests and learner should do (hopefully in a list)]

We demonstrate that a Bayseian approach to intrinsic motivation can share the characterizing properties of empowerment while maintaining the traits outlined by Schmidhuber.  
...
In this section, we show that in the single target state intrinsic motivation problem formulated above, expected reward maximization is less computationally complex than exact empowerment maximization. 
The variational approach from \cite{mohamed2015variational}, simplifies the computation of empowerment based strategies using mini-batches and Monte Carlo approximation, hence this is \textit{not} in direct comparison with the cost of the algorithm used in that paper. 
...
Since $\text{conOpt}$ must be at least linear in its second argument and monotone increasing in its first argument, maximizing expected reward is asymptotically less costly than maximizing empowerment. [TODO under what assumptions?]
